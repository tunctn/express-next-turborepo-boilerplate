...
import { type AccessControlMiddlewareOptions, accessControlMiddleware } from "@/middlewares/access-validators";
import { type RequestWithAuth, type RequestWithLooseAuth, authMiddleware } from "@/middlewares/auth.middleware";
import { type RequestValidation, validationMiddleware } from "@/middlewares/validation.middleware";
import { type AuthUser, COOKIE_NAME, DEFAULT_LOCALE, type Locale, type UserRole } from "@packages/shared";
...
type LooseAuthenticatedRequest = { user?: AuthUser };
...
export class Controller<ExcludedMethods extends string = never, ReqType = BaseRequest, ResType = unknown> {
  ...
  public useRedis = (middleware?: RedisMiddleware<ReqType & Omit<RedisOptions<ReqType>, "redisMiddleware">>) => {
    ...
    if (middleware) {
      ...
      this.options.withRedis.redisMiddleware = (_) => ({});
    }
      ...
      try {
        ...
        try {
          if (this.options.withRedis && this.options.ignoreRedis === false) {
            ...
            if (cachedResponse) {
              const { status, body, cached_at, expire } = JSON.parse(cachedResponse) as RedisResponseBody;
        ...
        if (this.options.withRedis) {
          ...
          // Return response
          res.status(res.statusCode).json(redisResponse);
          return;
        }
        await executeFn({ req: req as ReqType, res, next });
      } catch (err: any) {
